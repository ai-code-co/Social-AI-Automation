...
import { AtSign, BadgeCheck, ExternalLink, KeyRound, Loader2, Save, Share2, Trash2 } from 'lucide-react';
    ...
    <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      <section className="space-y-4">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-semibold tracking-normal text-slate-950 dark:text-white">Connected accounts</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">
            Add the Facebook Page or Instagram Business account that scheduled posts should publish to for {brand.company_name}.
          </p>
        </div>
          ...
          <div className="mt-4 space-y-3">
            {accounts.length === 0 ? (
              ...
              accounts.map(account => {
                ...
                return (
                  <div key={account.id} className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
          <div className="flex items-center gap-3">
            <div className="grid size-10 place-items-center rounded-md bg-slate-950 text-white dark:bg-slate-800">
              <PlatformIcon size={18} aria-hidden="true" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-slate-950 dark:text-white">Manual Meta connection</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">Use Meta Graph API tokens while the OAuth flow is being finalized.</p>
            </div>
          </div>
          {saved && (
        ...
        <div className="mt-5 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full resize-y rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:bg-white focus:ring-4 focus:ring-teal-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:bg-slate-950 dark:focus:ring-teal-500/20"
            />
          </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <button
              onClick={handleOAuth}
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-4 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300 dark:hover:bg-slate-800"
            >
